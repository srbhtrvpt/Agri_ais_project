...
hx1 = ax.hexbin(x_df['values'], y_df['values'], C= data['curvature'], cmap='inferno', gridsize= (160,120))
ax.set_xlabel("x")
ax.set_ylabel("y")
...
hx2 = ax.hexbin(x_df['values'], y_df['values'], C= data['intensity'], cmap='inferno', gridsize= (160,120))
ax.set_xlabel("x")
ax.set_ylabel("y")
...
hx = ax.hexbin(binned_data['intensity'], binned_data['curvature'],  C= probs.max(1), cmap=CM.jet )
ax.set_xlabel("intensity")
ax.set_ylabel("curvature")
fig11.colorbar(hx, ax=ax, label = 'probability')
fig11.suptitle("curvature vs intensity gmm heatmap binned")
fig11.savefig(path + "int_vs_curv_gmm_binned.png", dpi=400)