...
*/
    ...
    std::vector<std::string> input_path_split, input_topic_split;
    boost::split(input_path_split, input_bag_path, boost::is_any_of("."), boost::token_compress_on);

    // initialize ros
    rosbag::Bag input_bag, output_bag;
    input_bag.open(input_bag_path, rosbag::bagmode::Read);
    output_bag.open(input_path_split[0] + "_integrated.bag", rosbag::bagmode::Write);
...
bool fillTfBufferFromBag(rosbag::View &view, ros::Duration bagDuration, tf2_ros::Buffer *tfBuffer)
{
    ...
    std::cout << std::endl
              << "parsed " << tf_count << " tf messages." << std::endl;
    return true;
}
