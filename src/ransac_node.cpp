...
void callback(const sensor_msgs::PointCloud2ConstPtr &cloud)
{
    ...
    //std::cout << cloud_in->points.size() << "......" << cloud_in->width << "......" << cloud_in->height  << "......" << cloud_in->width*cloud_in->height << "\n" ;
    ...
    if (cloud_in->isOrganized())
    {
        ...
        for (int rr = 0; rr < cloud_in->height; rr++)
        {
            ...
            marker.color.b = 0.0;*/
                ...
                //std::cout << "x" << nx << "y" << ny << "z" << nz << "......" << indices.size() << "curv" << curvature << "cc" << cc << "rr" << rr << "\n";
    ...
    else
    {
        ...
        for (int i = 0 ; i < cloud_normals->points.size(); i++)
        {
            ...
            geometry_msgs::Point p;
            if(isfinite(cloud_in->points[i].x) && isfinite(cloud_in->points[i].y) && isfinite(cloud_in->points[i].z) && isfinite(cloud_normals->points[i].normal_z) && isfinite(cloud_normals->points[i].normal_y) && isfinite(cloud_normals->points[i].normal_x))
            {
